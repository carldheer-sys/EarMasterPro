...
import { Bug, ChevronLeft, Info, Loader2, Minus, Pause, Play, Plus, RefreshCw, Settings, Square } from 'lucide-react'
...
import { isNativeApp, isNativeSessionUrl, loadNativeSessionCatalog, loadNativeSessionFromUrl } from './mobileNative'
      ...
      <div className="mx-auto flex min-h-full max-w-md flex-col">
        ...
        <div className="space-y-4 pb-8">
          ...
          <SettingCard title="Modes">
            <FastSelect label="Mode" value="custom" options={[{ value: 'custom', label: 'Custom' }, { value: 'qa', label: 'Q&A (coming soon)' }]} onChange={() => {}} />
            <p className="text-xs text-slate-400">Q&A mode will be implemented later.</p>
          </SettingCard>
        </div>
      </div>
    </main>
...
function App() {
  ...
  const [nativeDirectoryUri, setNativeDirectoryUri] = useState('')
  ...
  const ensureFreshAudioContext = useCallback(async ({ force = false, latencyHint = 'playback', label = 'audio', start = true } = {}) => {
    ...
    if (shouldReplace) {
      addDebugEvent('Fresh audio context requested', {
        label,
        force,
        toneState: Tone.context.state,
        rawState: rawBefore?.state,
        latencyHint,
        start
      })
  ...
  const refreshCatalog = useCallback(async () => {
    ...
    try {
      ...
      let nativeSessions = []
      if (isNativeApp()) {
        const nativeCatalog = await loadNativeSessionCatalog()
        nativeSessions = nativeCatalog.sessions
        setNativeDirectoryUri(nativeCatalog.directoryUri || '')
        addDebugEvent('Native sessions scanned', { count: nativeSessions.length, directory: nativeCatalog.directoryUri })
      }
      const sessions = [...nativeSessions, ...(bundled.sessions || [])]
      setCatalog(sessions)
      setSelectedUrl(prev => sessions.some(song => song.url === prev) ? prev : (sessions[0]?.url || ''))
      addDebugEvent('Catalog refreshed', { total: sessions.length, bundled: bundled.sessions?.length || 0, native: nativeSessions.length })
    } catch (err) {
      console.error(err)
      addDebugEvent('Catalog refresh failed', { message: err.message })
      setError('Could not load sessions. Add .eartrainer.json files to Saved_Sessions and refresh.')
    } finally {
      setLoading(false)
    }
  }, [addDebugEvent])
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const { sessionName: name, sessionData, files } = isNativeSessionUrl(selectedUrl)
          ? await loadNativeSessionFromUrl(selectedUrl)
          : await loadSessionFromUrl(selectedUrl)
        if (cancelled) return
        ...
        <header className="rounded-[2rem] border border-white/10 bg-white/[0.08] p-4 shadow-2xl shadow-black/30 backdrop-blur-xl">
          <div className="mb-4 flex items-center justify-between gap-2">
            <div className="flex min-w-0 items-center gap-2">
              <img src={`${import.meta.env.BASE_URL}icons/icon-192.png`} alt="" className="h-8 w-8 shrink-0 rounded-lg" />
              ...
              <FastSelect
                label={(
                  <span className="inline-flex items-center gap-1.5">
                    <span>Session</span>
                    {isNativeApp() && (
                      <button type="button" onPointerDown={e => e.stopPropagation()} onClick={e => { e.stopPropagation(); setShowSessionInfo(v => !v) }} className="rounded-full bg-white/10 p-1 shadow-lg active:scale-95">
          ...
          {isNativeApp() && showSessionInfo && (
            <div className="mb-4 rounded-2xl border border-sky-300/20 bg-sky-400/10 p-3 text-xs text-sky-100">
              Add user sessions in Files: On My iPhone → EarMasterPro → Saved_Sessions. Tap Refresh if a new file does not appear. Bundled sessions are inside the app package and are not visible in Files.
              {nativeDirectoryUri && <div className="mt-2 break-all text-sky-200/70">{nativeDirectoryUri}</div>}
            </div>
          )}
